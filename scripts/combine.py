...
if __name__ == "__main__":
	...
	for i in range(len(combine_names)):
		...
		beam = beams[i]
		with open("../nmt_model/joint_inference/d4j_result/{}.txt".format(combine_name), "r") as file:
			content_combine = file.readlines()
			content_combine = [factor.strip() for factor in content_combine]
		with open("../nmt_model/origin_onmt/d4j_result/{}.txt".format(copy_name), "r") as file:
