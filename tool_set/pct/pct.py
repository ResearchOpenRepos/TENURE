		...
		if "(" not in factor:
			if factor == "this":
				...
				if factor in local_info:
					...
					if factor in global_info:
						type = factor
						if not check_type_in_import(curr_clazz, type, global_info, jdk_info):
							type = None
					if not type and factor in jdk_info:
						type = factor
						if not check_type_in_import(curr_clazz, type, global_info, jdk_info):
							type = None
					if not type:
...
def search_clazz_fields(factor, package, global_info, jdk_info, in_jdk=False):
		...
		if clazz_name in part:
			...
			for clazz in clazz_list:
				...
				for super_clazz in clazz["super_class"]:
					type = search_clazz_fields(factor, super_clazz, global_info, jdk_info, True if i == 1 else False)
					if type:
						return type
	return None


def search_clazz_methods(factor, package, curr_clazz_object, global_info, jdk_info, in_jdk=False):
	clazz_name = package.split(".")[-1]
	factor_name = factor.split("(")[0]
		...
		if clazz_name in part:
			...
			for clazz in clazz_list:
				...
				if find_flag:
					return method_type, method_params
				for super_clazz in clazz["super_class"]:
					type, parameters = search_clazz_methods(factor, super_clazz, curr_clazz_object, global_info, jdk_info, True if i == 1 else False)
					if type:
...
def get_all_super_clazz(type, global_info, elements):
	if type in global_info:
		for clazz_factor in global_info[type]:
			elements += clazz_factor["super_class"]
			for super_type in clazz_factor["super_class"]:
				get_all_super_clazz(super_type, global_info, elements)


def check_return_type(method, type_info, corresponding_method, global_info):
	...
	for element in elements:
		...
		if element == "null":
			if method_return_type in ["byte", "short", "int", "long", "float", "double", "char", "boolean", "void"]:
				return False
		elif element == "number_set":
			if method_return_type in ["byte", "short", "int", "long", "float", "double", "char"]:
				return True
		elif element != method_return_type:
			if element in ["int", "Integer"] and method_return_type in ["int", "Integer"] or \
					element in ["double", "Double"] and method_return_type in ["double", "Double"] or \
					element in ["float", "Float"] and method_return_type in ["float", "Float"] or \
					element in ["char", "Character"] and method_return_type in ["char", "Character"]:
				return True
			else:
				super_clazzes = []
				get_all_super_clazz(element, global_info, super_clazzes)
				if method_return_type in super_clazzes:
					return True
				else:
					return False
	return True
		...
		for i, cand in enumerate(curr_candidates):
			...
			for factor in cand:
				...
				if not factor_with_specific_part:
					...
					continue
				expr_list, if_syntax_correct = get_expr(factor_with_specific_part, factor)
				...
				if status:
					status = check_return_type(factor_with_specific_part, type_info, factor, curr_global_info)
				if status:
